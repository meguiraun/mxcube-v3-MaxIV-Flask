...
def connect(data):
    print 'asdkyastf gasfksadbjd'
    print data
    #ocketio.emit('test', {'data': 'Welcome'}, namespace='/test')

def signalCallback(aSignal, aSender, *args):
	#logging.getLogger('HWR').info('Signal-event callback')
	#logging.getLogger('HWR').info((aSignal,str(aSender.__class__).split('.')[0]))
	#logging.getLogger('HWR').info(args)
	socketio.emit('test', {'data': 'Server generated  signal', 'signal': aSignal, 'sender':str(aSender.__class__).split('.')[0]}, namespace='/test')
	#logging.getLogger('HWR').info('Signal-event Sent')

class MockUps():
	def __init__(self, hwr):
		logging.getLogger('HWR').info('Mockups initialised')
		self.resolution = hwr.getHardwareObject("/resolution-mockup")
		self.resolution.connect(self.resolution,'deviceReady', signalCallback)
		self.resolution.connect(self.resolution,'positionChanged', signalCallback)
		socketio.emit('test', {'data': 'mockups initialised','value':3.14159}, namespace='/test')
		logging.getLogger('HWR').info('Signal-event Sent')

	def isReady(self):
		logging.getLogger('HWR').info('in Mockups isReady')
		print self.resolution.equipmentReady()
		return True#self.resolution.equipmentReady()

	def setResolution(self, newres):
		logging.getLogger('HWR').info('in Mockups setting new resolution')
		return self.resolution.newResolution(newres)
...
class SampleCentring():
	...
	def __init__(self, hwr):
		...
		self.video.connect("imageReceived", signalCallback)
		...
		for sig in signals.MaxLabMicrodiff_signals:
			self.diffractometer.connect(self.diffractometer, sig, signalCallback)
...
class Collection():
	def __init__(self, hwr):
		...
		#for sig in signals.BL9113MultiCollect_signals:
		#	self.beamline_setup.connect(self.diffractometer, sig, signalCallback)
